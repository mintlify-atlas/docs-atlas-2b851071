---
title: "How to Upload Files to Vaulx Using Presigned S3 URLs"
sidebarTitle: "Uploading"
description: "Upload files to Vaulx via presigned S3 PUT URLs — bytes go straight to storage. Supports single-step and resumable multipart uploads for large files."
---
...
<Steps>
  ...
  <Step title="Confirm the upload">
    `POST /api/upload/confirm/{fileID}` — marks the file as `active` and makes it visible in the browser.
